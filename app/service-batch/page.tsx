...
export default function ServiceBatchPage() {
  ...
  const onSubmit = (data: BatchFormData) => {
    ...
    setCartridges((prev) =>
      prev.map((cartridge) =>
        selectedCartridges.includes(cartridge.id) ? { ...cartridge, status: 'service' } : cartridge,
      ),
    );
  ...
  const getStatusBadge = (status: string) => {
    ...
    return <Badge className={`${statusInfo.color} text-white text-xs`}>{statusInfo.label}</Badge>;
  };
                  ...
                  <tr>
                    ...
                    <td>${
                      cartridge.status === 'service'
                        ? 'Требуется заправка/Сняты с обслуживания'
                        : cartridge.status
                    }</td>
                  </tr>
                `,
                  ...
                  <TableRow key={batch.id}>
                    ...
                    <TableCell className='text-right'>
                      <div className='flex gap-2 justify-end'>
                        ...
                        <Button variant='outline' size='sm' onClick={() => printBatch(batch)}>
